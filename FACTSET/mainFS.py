...
for column in df_px_fx_adj.columns:
    ...
    if mthd.iloc[0,1] == "inverse":
        df_px_fx_adj[column] =  (df_input_index_days[column].values/df_input_index_days[mthd["Ticker"]].values.transpose()/mthd.iloc[0,2]).transpose()
...
for date in df_adjpx.index:
    ...
    df_input_CAo_required1 = df_input_CAo[np.array(df_input_CAo["Effective Date"]>date) & np.array(df_input_CAo["Effective Date"]<=Opendate)]   
    divtempdenom = 0
       
    """Index level Calculation"""
    for columns in df_rebal_details["SEDOL-CHK"][df_rebal_details.index == prerebaldate]:
        divtempdenom = divtempdenom + (df_adjunits.loc[date,df_mapping.loc[columns,"RIC"]]*df_px_fx_adj.loc[date,df_mapping.loc[columns,"RIC"]])
    Index_Series[date] = divtempdenom/Divisor[date]
    """ Adj Px adn Adj Unit Calculation for Opendate"""
    if date == pd.to_datetime('2011-01-18'):
        adsad = 1
    #No CA condition
    if len(df_input_CAo_required1) == 0:
        if not (date in rebal_dates):
    ...
    #CA Adjustment required  
        ...
        if not (date in rebal_dates):
            for columns in df_rebal_details["SEDOL-CHK"][df_rebal_details.index == prerebaldate]:
                ...
                if (df_mapping.loc[columns,"RIC"] in np.array(df_input_CAo_required1["RIC"])):
                    ...
                    if not len(df_input_CAo_required[df_input_CAo_required["Action Type"]=="Cash Dividend"]) == 0:
                        ...
                        if mthd.iloc[0,1] == "inverse":
                            df_adjpx.loc[Opendate,df_mapping.loc[columns,"RIC"]] = float(np.array(df_adjpx.loc[Opendate,df_mapping.loc[columns,"RIC"]] - (df_input_CAo_required[df_input_CAo_required["Action Type"]=="Cash Dividend"]["Gross Amount"] / df_input_index_days.loc[date,mthd["Ticker"]].values/mthd.iloc[0,2])))
                        elif mthd.iloc[0,1] == "multiply":
                            df_adjpx.loc[Opendate,df_mapping.loc[columns,"RIC"]] = float(np.array(df_adjpx.loc[Opendate,df_mapping.loc[columns,"RIC"]] - (df_input_CAo_required[df_input_CAo_required["Action Type"]=="Cash Dividend"]["Gross Amount"] * df_input_index_days.loc[date,mthd["Ticker"]].values/mthd.iloc[0,2])))

                    
                    if not len(df_input_CAo_required[df_input_CAo_required["Action Type"]=="Stock Dividend"]) == 0:
                        df_adjpx.loc[Opendate,df_mapping.loc[columns,"RIC"]] = float(np.array(df_adjpx.loc[Opendate,df_mapping.loc[columns,"RIC"]]/df_input_CAo_required[df_input_CAo_required["Action Type"]=="Stock Dividend"]["Gross Amount"]))
                        df_adjunits.loc[Opendate,df_mapping.loc[columns,"RIC"]] = float(np.array(df_adjunits.loc[Opendate,df_mapping.loc[columns,"RIC"]]*df_input_CAo_required[df_input_CAo_required["Action Type"]=="Stock Dividend"]["Gross Amount"]))
                    
                    if not len(df_input_CAo_required[df_input_CAo_required["Action Type"]=="Stock Split"]) == 0:
                        df_adjpx.loc[Opendate,df_mapping.loc[columns,"RIC"]] = float(np.array(df_adjpx.loc[Opendate,df_mapping.loc[columns,"RIC"]]/df_input_CAo_required[df_input_CAo_required["Action Type"]=="Stock Split"]["Gross Amount"]))
                        df_adjunits.loc[Opendate,df_mapping.loc[columns,"RIC"]] = float(np.array(df_adjunits.loc[Opendate,df_mapping.loc[columns,"RIC"]]*df_input_CAo_required[df_input_CAo_required["Action Type"]=="Stock Split"]["Gross Amount"]))
            ...
            prerebaldate = date
            print("Index calculation for: " + str(date)) 
            for columns in df_rebal_details["SEDOL-CHK"][df_rebal_details.index == prerebaldate]:
                df_adjunits.loc[Opendate,df_mapping.loc[columns,"RIC"]] = df_rebal_details[df_rebal_details["SEDOL-CHK"] == columns].loc[prerebaldate,"Index Constituent Weights "]*Index_Series[date]/df_px_fx_adj.loc[prerebaldate,df_mapping.loc[columns,"RIC"]]
                df_adjpx.loc[Opendate,df_mapping.loc[columns,"RIC"]] = df_px_fx_adj.loc[date,df_mapping.loc[columns,"RIC"]]
    ...
    #Opendate = pd.to_datetime('2014-12-19')    
    #date = pd.to_datetime('2014-12-18')
    #columns = "6241029"
    ...
    Divisor[Opendate] = float(Divisor[date]*divtempnum/divtempdenom)
...
preceading_date = np.array(lt_index_bus_day)[np.array(lt_index_bus_day)<end_date][-1]
...
directory ="E:/Users/jasmeet.gujral/Documents/WorkSpace/Factset/Output/Publish " +date.to_pydatetime().strftime('%Y%m%d')+ "/"
...
with open(filename_nqd, 'w') as f_nqd:
    ...
    f_nqd.write(date.to_pydatetime().strftime('%Y%m%d')+"\t"+"FDSFTPR\t"+str(round(Index_Series[date],9))+"\t"+str(round(Index_Series[preceading_date],9))+"\n")    
    f_nqd.write(date.to_pydatetime().strftime('%Y%m%d')+"\t"+"FDSFTTR\t"+str(round(Total_Index_Series[date],9))+"\t"+str(round(Total_Index_Series[preceading_date],9))+"\n")    
