...
def covarianceCalculator(df_input_clean,calDate,incep_date):
   
    df_Cov_Prev =  pd.DataFrame(index = df_input_clean.columns,columns = df_input_clean.columns)
    df_Cov =  pd.DataFrame(index = df_input_clean.columns,columns = df_input_clean.columns)
   
    if calDate == incep_date:   
        """initialising the initial Cov"""
        startDate = df_input_clean[:calDate].index[-252]
        df_Cov_Initial = pd.DataFrame(index = df_input_clean.columns,columns = df_input_clean.columns)
        df_Cov_Initial.loc[:,:] = 0
        df_Cov_Initial.values[[np.arange(len(df_Cov_Prev.columns))]*2] = .001
        """asset series required for cov cal"""
        df_cov_asset =  df_input_clean[(df_input_clean.index >= startDate) & (df_input_clean.index <= calDate)]
        DF = np.power(.5,(1/252))
        ...
        df_cov_asset =  df_input_clean[(df_input_clean.index > lastDate) & (df_input_clean.index <= calDate)]
